...
func readList(lex *lexer, v reflect.Value, typeMap TypeMap) {
	...
	case reflect.Struct: // ((name value) ...)
		for !endList(lex) {
			...
			name := getFieldName(v.Type(), lex.text())
			lex.next()
...
var (
	fieldNameMapCache   map[reflect.Type]map[string]string
	fieldNameMapCacheMu sync.Mutex
)
...
func getFieldName(structType reflect.Type, tagName string) string {
	fieldNameMapCacheMu.Lock()
	defer fieldNameMapCacheMu.Unlock()

	cache, ok := fieldNameMapCache[structType]
	if !ok {
		cache = map[string]string{}

		for i := 0; i < structType.NumField(); i++ {
			tagName := structType.Field(i).Tag.Get("sexpr")
			if tagName != "" {
				cache[tagName] = structType.Field(i).Name
			}
		}

		fieldNameMapCache[structType] = cache
	}

	fieldName, ok := cache[tagName]
	if !ok {
		return tagName
	}
