...
func Example_slice() {
	Display("slice", []*int{new(int), nil}, LIMIT)
	// Output:
	// Display slice ([]*int):
	// (*slice[0]) = 0
	// slice[1] = nil
...
func Example_struct() {
	Display("x", struct{ x interface{} }{3}, LIMIT)
	// Output:
...
func Example_structMap() {
	...
	Display("m", m, LIMIT)
	// Output:
	// Display m (map[struct { foo int; bar int }]int):
	// m[key.foo = 12
	// key.bar = 34] = 56
}
...
func Example_movie() {
	...
	//!-strangelove
	Display("strangelove", strangelove, LIMIT)
...
func Test(t *testing.T) {
	...
	Display("os.Stderr", os.Stderr, LIMIT)
	// Output:
	...
	Display("rV", reflect.ValueOf(os.Stderr), LIMIT)
	// Output:
